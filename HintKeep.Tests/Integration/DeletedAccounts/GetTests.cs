...
namespace HintKeep.Tests.Integration.DeletedAccounts
{
    public class GetTests : IClassFixture<HintKeepWebApplicationFactory>
    {
        ...
        public async Task Get_WithoutAuthentication_ReturnsUnauthorized()
        {
            ...
            var response = await client.GetAsync("/api/deleted-accounts");
        ...
        [Fact]
        public async Task Get_WhenAuthenticated_ReturnsOkOnlyWithAuthenticatedUserDeletedAccounts()
        {
        ...
        [Fact]
        public async Task Get_WhenUserHasDeletedAccounts_ReturnsOnlyDeletedOnes()
        {
            var userId = Guid.NewGuid().ToString("N");
            var accounts = new[]
            {
