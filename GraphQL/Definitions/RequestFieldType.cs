    ...
    where TResult : class
{
    ...
    {
        Resolver = new FuncFieldResolver<TResult>(ResolveAsync);
    }

    protected abstract TRequest GetInput(IResolveFieldContext context);

    protected virtual async ValueTask<TResult?> ResolveAsync(IResolveFieldContext context)
    {
        ...
        try
        {
            ...
            if (ObjectValidator.TryValidateObject(input, validationContext, validationResults, validateAllProperties: true) && validationResults.Count == 0)
            {
                ...
                var result = await context
                    ...
                    .ExecuteAsync(input, context.CancellationToken);
